...
import { Pais, PaisRpta } from '../data/pais.model';
import { PaisesRepository } from '../data/paises.repository';


@Injectable({ providedIn: 'root' })
export class PaisesStateService {
    items = signal<Pais[]>([]);
    item = signal<Pais | null>(null);
    ...
    loadItems(page: number = 1) {
        this.spinner.show();
        this.paisesRepository.getAll().subscribe({
            next: (data:PaisRpta) => {
    ...
    loadItemById(id: number) {
        this.spinner.show();
        this.paisesRepository.getBydId(id).subscribe({
            next: (data) => {
    ...
    addItem(item: Pais, onSuccess?: () => void) {
        this.spinner.show();
        this.paisesRepository.create(item).subscribe({
            next: (data) => {
                this.loadItems();
    ...
    updateItem(id: number, item: Pais, onSuccess?: () => void) {
        this.spinner.show();
        this.paisesRepository.update(id, item).subscribe({
            next: () => {
                this.loadItems();
    ...
    postForm(item: Pais, id?: number, onSuccess?: () => void){
        if(id){
    ...
    deleteItem(id: number) {
        this.spinner.show();
        this.paisesRepository.delete(id).subscribe({
            next: () => {
                this.loadItems();
